...
open class Wallet private constructor(val masterXprvKey: ExtendedKey, privKeys: List<ExtendedKey> = listOf(), val coin: CoinType = CoinType.Bitcoin) : Event() {

    val childrenPrivateKeys = mutableListOf<ExtendedKey>()
     ...
     */
    init {
        privKeys.forEach { childrenPrivateKeys.add(it) }

        if (privKeys.size < childrenKeysAmount) {
            val begin = privKeys.size
            (begin..childrenKeysAmount).mapTo(childrenPrivateKeys) { masterXprvKey.derive(44).derive(coin.raw).derive(0).derive(0).derive(it) }
        }
    }
    ...
    companion object {
        ...
        fun fromMasterXprvKey(masterPrivateKey: ExtendedKey, childrenPrivateKeys: List<ExtendedKey> = listOf(), coin: CoinType = CoinType.Bitcoin) = Wallet(masterPrivateKey, childrenPrivateKeys, coin)

        fun fromMasterXprvKey(masterPrivateKey: String, childrenPrivateKeys: List<String> = listOf(), coin: CoinType = CoinType.Bitcoin): Wallet? {
            return try {
                val master = ExtendedKey.parse(masterPrivateKey, true)
                val children = childrenPrivateKeys.map { ExtendedKey.parse(it, true) }

                fromMasterXprvKey(master, children, coin)
            } catch (e: Exception) {
                null
            }
        }
    ...
    fun isValidTx(tx: Transaction): Boolean {
        return tx.txOuts.any { out ->
            ...
            return@any childrenPrivateKeys.any { it.ecKey!!.publicKeyHash!!.contentEquals(ops[2].second!!) }
        }
    }
}